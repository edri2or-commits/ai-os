...
## 2025-11-29 — DEC-011: Claude Desktop ⟷ HTTP MCP Integration

**Date:** 2025-11-29 | **Owner:** Or | **Status:** ✅ Operational | **Phase:** 2.3

### Context
AI-OS has MCP GitHub Server on localhost:8081 (HTTP API), designed for GPT Custom Actions.
Claude Desktop MCP expects stdio protocol.

**Problem:** How to connect Claude Desktop without:
1. Duplicating server logic (violates DRY)
2. Breaking multi-agent architecture

### Decision
**Claude Desktop → PowerShell → HTTP API (localhost:8081)**

- Method: Windows-MCP:Powershell-Tool
- Status: ✅ Operational (verified 2025-11-29 with PR #22)

### Rationale
1. DRY: One server for all agents (Claude, GPT, Chat1)
2. Proven: PR #22 created successfully
3. Multi-agent: Same infrastructure for all

### Impact
- All GitHub capabilities (GH-001 to GH-006) now ✅ Operational
- Updated: CAPABILITIES_MATRIX, SYSTEM_SNAPSHOT, AGENTS_INVENTORY, EVENT_TIMELINE, SERVICES_STATUS

### Related
DEC-002, DEC-003, DEC-007 | CONSTITUTION Laws #2, #7 | Test PR: #22
